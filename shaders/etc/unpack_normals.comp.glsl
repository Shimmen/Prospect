...
layout(binding = 1, rgba8)   restrict writeonly uniform image2D img_target;
...
void main()
{
    ...
    if (lengthSquared(packedNormal) > 0.01)
    {
        ...
        imageStore(img_target, pixelCoord, color);
    }
    else
    {
        imageStore(img_target, pixelCoord, vec4(0.0, 0.0, 0.0, 1.0));
    }
}
