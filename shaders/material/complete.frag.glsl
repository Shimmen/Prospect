...
void main()
{
    ...
    // It seems like NaNs can be generated by the normal mapping if the tangent is missing.
    // I'm not sure why it's missing (it's only for a single triangle), but this is an easy fix.
    vec2 encodedNormal = any(isnan(N)) ? vec2(0.0) : octahedralEncode(N);
    o_g_buffer_norm_vel = vec4(encodedNormal, screenSpaceVelocity);
}
